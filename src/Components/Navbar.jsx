  ...
  return (
    <nav className="navbar navbar-expand-lg bg-light">
      <div className="container-fluid">
        <Link className="navbar-brand" to="/tuprode">TU PRODE</Link>
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav">
            ...
            {links.map(link => <li key={link.name} className='nav-item'><Link className='nav-link' to={link.url}>{link.name.toUpperCase()}</Link></li>)}
          </ul>
        </div>
      </div>
    </nav>
