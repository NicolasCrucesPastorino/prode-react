...
export const useProde = (torneo={}, puntos=[]) => {
    ...
    function getPartidoById(id) {
        ...
        return _resultados.find(grupo => grupo.nombre === groupName).partidos.find(partido => partido.partidoid === id)
    }

    function updatePartido(partidoId, golesequipoA='', golesequipoB='') {
        const partido = getPartidoById(partidoId)
        _setResultados(_resultados.map(grupo => {
